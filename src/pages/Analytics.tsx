...
export default function Analytics() {
  const horizonPerformance = [
    { horizon: "15min", direction: 89, mape: 0.15, confidence: 87 },
    { horizon: "1hr", direction: 84, mape: 0.24, confidence: 81 },
    { horizon: "4hr", direction: 78, mape: 0.45, confidence: 74 },
    { horizon: "12hr", direction: 72, mape: 0.82, confidence: 68 },
    { horizon: "24hr", direction: 68, mape: 1.2, confidence: 65 },
    { horizon: "3d", direction: 62, mape: 2.1, confidence: 58 },
    { horizon: "7d", direction: 55, mape: 3.5, confidence: 52 },
  ];
  ...
  const scatterData = Array.from({ length: 50 }, () => {
    const predicted = 50000 + Math.random() * 5000;
    const actual = predicted + (Math.random() - 0.5) * 2000;
    return { predicted, actual, horizon: ["15m", "1h", "4h", "24h"][Math.floor(Math.random() * 4)] };
  });

  const trainingHistory = [
    { date: "2024-01-15", type: "Full", duration: "4.2 hrs", dataPoints: 2450000, before: 72.3, after: 78.5, notes: "Added sentiment features" },
    { date: "2024-01-08", type: "Incremental", duration: "45 min", dataPoints: 125000, before: 70.1, after: 72.3, notes: "Weekly update" },
    { date: "2024-01-01", type: "Full", duration: "5.1 hrs", dataPoints: 2300000, before: 68.4, after: 70.1, notes: "New transformer architecture" },
    { date: "2023-12-25", type: "Incremental", duration: "38 min", dataPoints: 98000, before: 67.2, after: 68.4, notes: "Holiday patterns" },
    { date: "2023-12-18", type: "Full", duration: "4.8 hrs", dataPoints: 2100000, before: 65.0, after: 67.2, notes: "Volume indicators added" },
  ];
  ...
  return (
    <div className="p-4 lg:p-6 space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="flex items-start justify-between">
              <div>
                ...
                <div className="flex items-baseline gap-2 mt-1">
                  <span className="text-3xl font-bold text-success">{modelPerformance.directionAccuracy}%</span>
          ...
          <CardContent className="p-6">
            <div className="flex items-start justify-between">
              <div>
                ...
                <div className="flex items-baseline gap-2 mt-1">
                  <span className="text-3xl font-bold">{modelPerformance.directionAccuracy}%</span>
                </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-start justify-between">
              <div>
                ...
                <div className="flex items-baseline gap-2 mt-1">
                  <span className="text-3xl font-bold font-mono">${modelPerformance.mae}</span>
                </div>
                <div className="mt-2">
                  <div className="flex justify-between text-xs mb-1">
                    <span className="text-muted-foreground">Target: $100</span>
                    <span>{Math.round((100 / modelPerformance.mae) * 100)}%</span>
                  </div>
                  <Progress value={(100 / modelPerformance.mae) * 100} className="h-1.5" />
                </div>
              </div>
        ...
        <Card className="bg-card border-border metric-card">
          <CardContent className="p-6">
            <div className="flex items-start justify-between">
              <div>
                <p className="text-sm text-muted-foreground">Model Age</p>
                <div className="flex items-baseline gap-2 mt-1">
                  <span className="text-3xl font-bold">3</span>
                  <span className="text-lg text-muted-foreground">days</span>
                </div>
                <div className="text-xs text-muted-foreground mt-1">
                  <div>Last: Jan 15, 2024</div>
                  <div>Next: <span className="text-primary">Jan 22, 2024</span></div>
                </div>
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className="bg-card border-border">
          <CardHeader>
            <CardTitle>Accuracy Over Time (30 Days)</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="h-[300px]">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={accuracyOverTime} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                  ...
                  <Line type="monotone" dataKey="shortTerm" name="Short-term (15m-4h)" stroke="hsl(var(--success))" strokeWidth={2} dot={false} />
                  <Line type="monotone" dataKey="longTerm" name="Long-term (24h-7d)" stroke="hsl(var(--warning))" strokeWidth={2} dot={false} />
                </LineChart>
              </ResponsiveContainer>
            </div>
          </CardContent>
        ...
        <Card className="bg-card border-border">
          <CardHeader>
            <CardTitle>Predicted vs Actual Price</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="h-[300px]">
              <ResponsiveContainer width="100%" height="100%">
                <ScatterChart margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                  ...
                  />
                  <ReferenceLine 
                    segment={[{ x: 50000, y: 50000 }, { x: 55000, y: 55000 }]} 
                    stroke="hsl(var(--primary))" 
                    strokeDasharray="5 5"
                    strokeWidth={2}
                  />
                  <Scatter name="Predictions" data={scatterData} fill="hsl(var(--primary))" opacity={0.6} />
                </ScatterChart>
              </ResponsiveContainer>
            </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-border">
                  ...
                  <th className="text-right py-3 px-4 text-sm font-semibold text-muted-foreground">Accuracy Before</th>
                  <th className="text-right py-3 px-4 text-sm font-semibold text-muted-foreground">Accuracy After</th>
